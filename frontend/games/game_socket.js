...
const gameSocketConfig = (socketId) => {
  ...
  gameSocket.on(GAME_CONSTANTS.GAME_INFO, (data) => {
    ...
    for (const player of data.gameInfo) {
      const template = document
        .querySelector("#opponent-template")
  ...
  gameSocket.on(GAME_CONSTANTS.FACE_UP_CARD, (data) => {
    ...
    const template = document
      .querySelector("#board-card-template")
      .content.cloneNode(true);
    ...
    if (data.faceUpCard.modifier === "none") {
      ...
      cardTop.innerText = data.faceUpCard.value;
      cardBottom.innerText = data.faceUpCard.value;
      cardCenter.innerText = data.faceUpCard.value;
      cardColor.classList.add("card-" + data.faceUpCard.color);
    } else if (data.faceUpCard.color === "none") {
      // p.innerText = data.faceUpColor.current_color;
      cardColor.classList.add("card-" + data.faceUpColor.current_color);
    } else {
      cardTop.innerText = data.faceUpCard.modifier;
      cardBottom.innerText = data.faceUpCard.modifier;
      cardColor.classList.add("card-" + data.faceUpCard.color);
