...
const sendGameState = async (io, gameId) => {
  ...
  io.to(gameSocketId).emit(GAME_CONSTANTS.GAME_INFO, { gameInfo });  
  ...
  io.to(gameSocketId).emit(GAME_CONSTANTS.USER_CURRENT, { currentPlayer });
  ...
  io.to(gameSocketId).emit(GAME_CONSTANTS.FACE_UP_CARD, {faceUpCard, faceUpColor});
